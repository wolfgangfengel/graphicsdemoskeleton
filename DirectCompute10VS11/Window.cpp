...
static float GetUniform()
{
    ...
    return (u) * 2.328306435454494e-10 * 2.0f;
}
...
//	temp.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT | DXGI_USAGE_UNORDERED_ACCESS | DXGI_USAGE_SHADER_INPUT;
    ...
    Desc.MiscFlags = 0;
    Desc.ByteWidth = 128;//((29  + 15)/16)*16; //((sizeof( QJulia4DConstants ) + 15)/16)*16; // must be multiple of 16 bytes
...
//	pSwapChain->GetBuffer(0, (REFIID ) &IID_ID3D11Texture2D, (LPVOID*)&(pBackBuffer) ) ;
	...
	pd3dDevice->CreateShaderResourceView((ID3D11Resource *) pStructuredBuffer, &sbSRVDesc, &pComputeShaderSRV);
...
//	ID3DBlob *pCompressedByteCodeBlob = NULL;
	ID3D11ComputeShader *pCompiledComputeShader = NULL;
 	...
 	HRESULT hr = D3DX11CompileFromFile(L"qjulia4D.hlsl", NULL, NULL, "CS_QJulia4D", "cs_4_1", 0, 0, NULL, &pByteCodeBlob, &pErrorBlob, NULL);
...
//	D3DDecompressShaders(pCompressedByteCodeBlob->lpVtbl->GetBufferPointer(pCompressedByteCodeBlob), sizeOfCompressedBlob, 1, 0, NULL, 0, &pByteCodeBlob, NULL);
	...
	hr =
#endif 
	pd3dDevice->CreateComputeShader(pByteCodeBlob->GetBufferPointer(), pByteCodeBlob->GetBufferSize(), NULL, &pCompiledComputeShader);
 ...
 */			
	...
	// release all D3D device related resources
		...
		pStructuredBuffer->Release();
		pComputeOutputUAV->Release();
		pComputeShaderSRV->Release();
