...
void PostFX( uint3 Gid : SV_GroupID, uint3 DTid : SV_DispatchThreadID, uint3 GTid : SV_GroupThreadID, uint GI : SV_GroupIndex  )
{
	...
	// index in x (1920) goes from 0 to 59 | 60 (thread groups) * 16 (threads) = 960 indices in x
	// index in y (1080) goes from 0 to 33 | 34 (thread groups) * 32 (threads) = 1088 indices in y
	uint idx = ((DTid.x * 2) + DTid.y * c_width);

	// 1920 * 1080 = 2073600 pixels
	// 60 * 34 * 512 (number of threads : 8 * 32) * 2 (number of fetches) = 2088960
	sharedMem[GI] = (dot(Input[idx], LumVector) + dot(Input[idx + 1], LumVector));
	
	// wait until everything is transfered from device memory to shared memory
	GroupMemoryBarrierWithGroupSync();

	// hard-coded for 512 threads
	if (GI < 256)
