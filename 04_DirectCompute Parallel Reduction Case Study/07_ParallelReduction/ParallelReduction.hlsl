...
void PostFX(uint3 ThreeDIndexOfThreadGroupInDispatch : SV_GroupID, uint3 DTid : SV_DispatchThreadID, uint3 ThreeDIndexOfThreadInGroup : SV_GroupThreadID, uint IndexOfThreadInGroup : SV_GroupIndex)
{
	...
	// index in x (1920) goes from 0 to 30 (thread groups) * 16 (threads) = 480 indices in x
	// index in y (1080) goes from 0 to 17 (thread groups) * 64 (threads) = 1088 indices in y
	...
	if (IndexOfThreadInGroup < 512)
		sharedMem[IndexOfThreadInGroup] += sharedMem[IndexOfThreadInGroup + 512];
	GroupMemoryBarrierWithGroupSync();

	if (IndexOfThreadInGroup < 256)
		sharedMem[IndexOfThreadInGroup] += sharedMem[IndexOfThreadInGroup + 256];
	GroupMemoryBarrierWithGroupSync();
