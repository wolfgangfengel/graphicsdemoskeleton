...
ID3D12CommandAllocator* mCommandAllocator[FRAMECOUNT];
ID3D12CommandQueue* mCommandQueue;
ID3D12DescriptorHeap* mDescriptorHeap;
ID3D12PipelineState* mPSO;
ID3D12GraphicsCommandList* mCommandList[FRAMECOUNT];
...

{
	...
	{ // Extra scope to make compiler accept the __decalspec(naked) with local variables
	  ...
	  // timer global variables
		...
		// Create a RTV for each back buffer in the swap chain
		for (UINT n = 0; n < FRAMECOUNT; n++)
		{
			...
			mDevice->lpVtbl->CreateCommandAllocator(mDevice, D3D12_COMMAND_LIST_TYPE_DIRECT, (REFIID)&IID_ID3D12CommandAllocator, (LPVOID*)(&mCommandAllocator[n]));
			mDevice->lpVtbl->CreateCommandList(mDevice, 0, D3D12_COMMAND_LIST_TYPE_DIRECT, mCommandAllocator[n], mPSO, (REFIID)&IID_ID3D12CommandList, (LPVOID*)(&mCommandList[n]));
		}
		...
		for (UINT n = 0; n < FRAMECOUNT; n++)
		{
			...
			ThrowIfFailed(mCommandList[n]->lpVtbl->Close(mCommandList[n]));
		}
		...
		while (BRunning)
		{
			...
			ID3D12GraphicsCommandList* CommandList = mCommandList[mframeIndex];
			ID3D12CommandAllocator* CommandAllocator = mCommandAllocator[mframeIndex];
			...
			// re-recording.
			ThrowIfFailed(CommandList->lpVtbl->Reset(CommandList, CommandAllocator, mPSO));

			CommandList->lpVtbl->SetGraphicsRootSignature(CommandList, mRootSignature);
			CommandList->lpVtbl->RSSetViewports(CommandList, 1, &mViewport);

			// this is now a requirement to use, although we set the viewport above
			CommandList->lpVtbl->RSSetScissorRects(CommandList, 1, &mRectScissor);
			...
			CommandList->lpVtbl->IASetVertexBuffers(CommandList, 0, 1, &mDescViewBufVert);
		...
		// release all objects allocated for each the render targets
		for (UINT n = 0; n < FRAMECOUNT; n++)
		{
			WaitForFence(mFences[n], mFenceValues[n], mframeFenceEvents[n]);
			CloseHandle(mframeFenceEvents[n]);
			mFences[n]->lpVtbl->Release(mFences[n]);

			mRenderTarget[n]->lpVtbl->Release(mRenderTarget[n]);
			mCommandAllocator[n]->lpVtbl->Release(mCommandAllocator[n]);
			mCommandList[n]->lpVtbl->Release(mCommandList[n]);
		}
